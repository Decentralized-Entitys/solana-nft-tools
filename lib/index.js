...
async function getMintTokensByOwner (conn, walletAddr, cacheTtlMins = null) {
  ...
  let data=await conn.getParsedTokenAccountsByOwner(
  ...
  let tokenAccs=[]
  tokenAccs=data.value
  console.log(tokenAccs[0].account.data.parsed.info.tokenAmount.amount)
  let FullOwned=tokenAccs.filter(eachObj =>eachObj.account.data.parsed.info.tokenAmount.amount=== "1");

  tokens = FullOwned.map(t => t.account.data.parsed.info.mint);
