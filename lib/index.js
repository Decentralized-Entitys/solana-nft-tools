...
async function getMintTokensByOwner (conn, walletAddr, cacheTtlMins = null) {
  ...
  let tokenAccs =await conn.getParsedTokenAccountsByOwner(toPublicKey(walletAddr),{ programId: toPublicKey(TOKEN_PROGRAM_ID) },COMMITMENT_PROCESSED);
  let RealArray=[]
  tokenAccs=tokenAccs["value"].forEach(element => {
    RealArray.push(element["account"]["data"]["parsed"])
  });
  let FullOwned=RealArray.filter(eachObj => eachObj.info.tokenAmount.amount === "1");

  tokens = await FullOwned.map(t => t.info.mint);
...
async function GetMintTokensIdOnly (conn, walletAddr, cacheTtlMins = null) {
  const cacheKey = `GetMintTokensIdOnly-${walletAddr}`;
  let tokens = cache.get(cacheKey);
  if (tokens) {
    return tokens;
  }
...
exports.GetMintTokensIdOnly = GetMintTokensIdOnly;
